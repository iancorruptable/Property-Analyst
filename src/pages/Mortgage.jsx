...
export default function Mortgage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Mortgage & Equity Tracker</h1>
        <p className="text-sm text-slate-500 mt-1">Module 4 — Loan balance, equity position, amortization</p>
      </div>
              ...
              <div className="space-y-3">
                {pieData.map(item => (
                  <div key={item.name} className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-600">{item.name}: <span className="font-semibold text-slate-900">{formatCurrency(item.value)}</span></span>
                  </div>
                ))}
                <div className="text-xs text-slate-400">Based on {latestValue === purchasePrice ? 'purchase price' : 'latest estimate'}</div>
              </div>
            </div>
          ) : (
            <p className="text-sm text-slate-400 text-center py-8">Enter property value and loan balance to see equity breakdown.</p>
          )}
        </Card>
        ...
        <Card title="Loan Details">
          <div className="space-y-2">
            {[
              ...
              <div key={label} className="flex justify-between py-1 border-b border-slate-50 text-sm">
        ...
        <Card title="Escrow Breakdown (Monthly)">
          <div className="space-y-2">
            {[
              ...
              <div key={label} className="flex justify-between py-1 border-b border-slate-50 text-sm">
                <span className="text-slate-600">{label}</span>
                <span className="font-medium text-slate-900">{value > 0 ? formatCurrency(value) : '—'}</span>
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card title="LTV Health Guide">
          <div className="space-y-2">
            {[
              ...
              <div key={item.range} className={`flex items-center justify-between py-2 px-3 rounded-lg text-sm ${item.current ? 'bg-slate-100 font-semibold' : ''}`}>
                <div className="flex items-center gap-2">
                  <StatusBadge status={item.status}>{item.range}</StatusBadge>
                  <span className="text-slate-700">{item.label}</span>
                </div>
                {item.current && <span className="text-xs text-blue-600 font-bold">← Current</span>}
              </div>
            ))}
          </div>
        </Card>
